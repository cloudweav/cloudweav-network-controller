...
import (
	...
	"github.com/cloudweav/cloudweav-network-controller/pkg/controller/agent"
	"github.com/cloudweav/cloudweav-network-controller/pkg/controller/manager"
)
...
func main() {
	app := cli.NewApp()
	app.Name = "cloudweav-network-controller"
	app.Version = VERSION
	app.Usage = "Cloudweav Network Controller, to help with cluster network configuration. Options kubeconfig or masterurl are required."
	commonFlags := []cli.Flag{
		...
		cli.StringFlag{
			Name:   "helper-image",
			EnvVar: "HELPER_IMAGE",
			Value:  "rancher/cloudweav-network-helper:master-head",
			Usage:  "The image of cloudweav network helper, defaults to rancher/cloudweav-network-helper.",
		},
	}
...
func run(c *cli.Context, registerFuncList []config.RegisterFunc, leaderelection bool) error {
	...
	if err != nil {
		klog.Fatalf("Error building cloudweav controllers: %s", err.Error())
	}
	...
	if leaderelection {
		leader.RunOrDie(ctx, "kube-system", "cloudweav-network-controllers", client, callback)
	} else {
		callback(ctx)
	}
