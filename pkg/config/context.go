...
func SetupManagement(ctx context.Context, restConfig *rest.Config, options *Options) (*Management, error) {
	...
	cloudweavNetwork, err := ctlnetwork.NewFactoryFromConfigWithOptions(restConfig, opts)
	if err != nil {
		return nil, err
	}
	management.CloudweavNetworkFactory = cloudweavNetwork
	management.starters = append(management.starters, cloudweavNetwork)
